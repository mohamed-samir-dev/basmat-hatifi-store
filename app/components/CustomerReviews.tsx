...
export default function CustomerReviews() {
  ...
  return (
    <section className="w-full bg-teal-100 py-6" dir="rtl">
    <div className="max-w-6xl mx-auto px-3 sm:px-4">
      <div className="flex items-center gap-2 sm:gap-3 mb-6">
        <div className="flex-1 h-px bg-gray-300" />
        <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-gray-700 whitespace-nowrap px-2 sm:px-3">
          آراء العملاء
        </h2>
        <div className="flex-1 h-px bg-gray-300" />
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {submitted ? (
          <span className="text-xs sm:text-sm text-green-600 font-semibold px-3 py-1.5 rounded-full border border-green-200 bg-green-50">
            ✓ تم إرسال تعليقك وسيظهر بعد المراجعة
          </span>
        ) : (
          <button
            onClick={() => setShowForm((v) => !v)}
            className="text-xs sm:text-sm font-semibold text-purple-600 hover:text-purple-800 whitespace-nowrap px-3 py-1.5 rounded-full border border-purple-200 hover:bg-purple-50 transition-colors"
          >
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mt-4 bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col gap-3">
          ...
          <textarea
            ...
            rows={3}
            className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-400 resize-none"
            required
          />
          ...
          <button type="submit" disabled={submitting}
            className="bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold py-2 rounded-lg transition-colors disabled:opacity-60">
