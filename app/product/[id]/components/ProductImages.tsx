    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-3 sm:p-6">
      ...
      {images.length > 1 && (
        <div className="flex gap-2 sm:gap-3 mt-3 sm:mt-4 justify-center flex-wrap">
          {images.map((img, i) => (
            <button key={i} onClick={() => setSelectedImage(i)} className={`relative w-12 h-12 sm:w-16 sm:h-16 rounded-lg overflow-hidden border-2 transition ${i === selectedImage ? "border-emerald-500" : "border-gray-200 hover:border-gray-300"}`}>
