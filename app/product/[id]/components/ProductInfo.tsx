...
export default function ProductInfo({ product, addedToCart, onAddToCart }: ProductInfoProps) {
  ...
  const { name, brand, color, storage, network, salePrice, taxIncluded, installment, freeDelivery, deliveryTime, warrantyYears, inStock } = product;
  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-4">
      <div className="bg-white rounded-2xl border border-gray-100 p-3 sm:p-5">
        {brand && <span className="text-xs sm:text-sm text-emerald-600 font-semibold">{brand}</span>}
        ...
        {(color || storage || network) && (
          <div className="flex gap-2 mt-2 sm:mt-3 flex-wrap">
            {[color, storage, network].filter(Boolean).map((t, i) => (
              <span key={i} className="text-xs bg-gray-100 text-gray-600 px-2 sm:px-3 py-1 rounded-full">{t}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3">
        <div className="bg-white rounded-xl border border-gray-100 px-3 sm:px-4 py-2.5 sm:py-3 flex items-center gap-2 sm:gap-3">
          <IoCarOutline size={18} className="text-emerald-500 shrink-0" />
        ...
        <div className="bg-white rounded-xl border border-gray-100 px-3 sm:px-4 py-2.5 sm:py-3 flex items-center gap-2 sm:gap-3">
          <IoTimeOutline size={18} className="text-emerald-500 shrink-0" />
      ...
      {!addedToCart ? (
        <button
          onClick={onAddToCart}
          className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm sm:text-base py-3 sm:py-3.5 rounded-xl flex items-center justify-center gap-2 transition-colors"
        >
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-center gap-2 text-emerald-600 bg-emerald-50 py-2.5 rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            <button
              onClick={() => router.back()}
              className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-xs sm:text-sm py-2.5 sm:py-3 rounded-xl transition-colors"
            >
              متابعة التسوق
            </button>
            <button
              onClick={() => router.push("/cart")}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm py-2.5 sm:py-3 rounded-xl flex items-center justify-center gap-2 transition-colors"
            >
