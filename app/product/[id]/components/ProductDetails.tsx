  ...
  return (
    <>
      {installment?.available && (
        <div className="bg-white rounded-2xl border border-gray-100 p-3 sm:p-5 mt-4 sm:mt-6">
          <h3 className="text-sm sm:text-base font-bold text-gray-800 mb-3 sm:mb-4">💳 التقسيط</h3>
          <div className="bg-emerald-50 rounded-xl p-3 sm:p-4 mb-3 sm:mb-4">
            ...
            {installment.note && <p className="text-xs text-emerald-600 mt-1">{installment.note}</p>}
          </div>
          {installment.policy && (
              ...
              <div className="flex flex-col gap-2">
                {installment.conditions.map((c, i) => (
                  <div key={i} className="flex items-start gap-2 text-xs sm:text-sm text-gray-600">
                    <IoCheckmarkCircle size={14} className="text-emerald-500 shrink-0 mt-0.5" />
                    <span>{c}</span>
                  </div>
                ))}
              </div>
      ...
      {specs && Object.values(specs).some(Boolean) && (
        <div className="bg-white rounded-2xl border border-gray-100 p-3 sm:p-5 mt-3 sm:mt-4">
          <h3 className="text-sm sm:text-base font-bold text-gray-800 mb-3">المواصفات</h3>
          <div className="rounded-xl overflow-hidden border border-gray-100">
            {specLabels.map(([key, label], i) =>
              specs[key] ? (
                <div key={key} className={`flex text-xs sm:text-sm px-3 sm:px-4 py-2.5 sm:py-3 ${i % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
